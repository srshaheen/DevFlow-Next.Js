        ...
        return (
          <SheetClose asChild key={item.route}>
            <Link
              ...
              } flex items-center justify-self-auto gap-4 bg-transparent p-4`}
            >
              <Image
                src={item.imgURL}
                alt={item.label}
                width={20}
                height={20}
                className={`${isActive ? "" : "invert-100 dark:invert-0"}`}
              />
    ...
    <Sheet>
      <SheetTrigger asChild>
        <Image
          ...
          width={36}
          height={36}
          className="sm:hidden invert-100 dark:invert-0"
        />
      </SheetTrigger>
      <SheetContent side="left" className="background-light900_dark200 border-none px-4 pt-12">
